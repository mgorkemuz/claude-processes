...
If `$ARGUMENTS` is empty, default to `claude-processes unstash --latest --attach` — respawn the most recent stash AND register it with the current session so it keeps showing up in `/shepherd:processes`. Report which stash that was (command + cwd).
